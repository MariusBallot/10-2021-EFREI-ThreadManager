...
class MainThreadManager {
  ...
  init(threadsData, canvas) {
    ...
    const isDoable = this.doablility(threadsData)
    if (isDoable) {
      ...
      this.computeBusyPeriode(this.threadsData)
      ...
      console.log("this is not doable")
  }

  computeBusyPeriode(threadsData) {
    let prevW = 0
  ...
  doablility(data) {
    let flag = false
    //const URM = 1 / (Math.pow(2, data.length) - 1)
    let U = 0
    data.forEach(thread => {
